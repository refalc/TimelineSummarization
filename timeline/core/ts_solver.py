...
class TSSolver:
    ...
    def construct_timeline_summary(self, timeline_queries, timeline_collection):
        ...
        for time, collection in timeline_collection.iterate_collections_by_time():
            ...
            sentences = self._get_sentences_from_collection(collection)
    ...
    def _extract_top_sentence(self, sentences, query, all_extracted, today_extracted):
        ...
        for sent in sentences:
            sent_score = self._rank_one_sentence(sent, query, all_extracted, today_extracted)
    ...
    def _rank_one_sentence(self, sentence, query, all_extracted, today_extracted):
        lambda_val = self.m_Config['lambda']
        sim_to_query = self._calc_sim(query, sentence)# query.sim(sentence)
        ...
        for sent_pair in all_extracted:
            ...
            sim_to_extracted = max(sim_to_extracted, self._calc_sim(sentence, sent_pair[0]))
        ...
        sentence_num_penality = 1.0 - 0.5 * math.sin(
